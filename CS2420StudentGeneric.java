...
public class CS2420StudentGeneric<Type> extends UofUStudent {
	...
	private double calculateWeightedPercentage(ArrayList<Double> theScore, double thePercent, Boolean dropLowest) {
		...
		if (dropLowest) {
			...
			return calculateWeightedPercentage(droppedList, thePercent, false); // saves on code by pretending its not
																				// dropped.
	...
	/**
	 * Computes the final percentage of all scores for a student. Will not function
	 * properly without 1 score in assignments and exams, 2 scores in labs and
	 * quizzes.
	 * 
	...
	public double computeFinalScore() {
		...
		// calculate the final score. Do this by calculating percentage of type,
		// multiply by weight, add all together.
		double totalScore = 0.0;
		...
		return totalScore * 100; // the 100 is for bringing to what the number looks as a % eg 99.9% rather than
									// .999
	}

	/**
	 * Calculates what the final letter grade of all your different scores will be.
	 * uses computeFinalScore().
	 * 
	 * @return
	 */
